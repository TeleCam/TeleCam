...
package ir.irani.telecam.ui.ActionBar;
...
public class ActionBar extends FrameLayout {
    ...
    private void createsubtitleTextView() {
        ...
        subtitleTextView.setTypeface(AndroidUtilities.getTypeface(FontChanger.getFont()));
        subtitleTextView.setTextColor(Theme.ACTION_BAR_SUBTITLE_COLOR);
        addView(subtitleTextView, 1, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP));
    }
    ...
    public void changeGhostModeVisibility() {
        if (Setting.getGhostMode() && true) {//showghostmodeicon
    ...
    private void createTitleTextView() {
        ...
        titleTextView.issans=true;
        titleTextView.setGravity(Gravity.LEFT);
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                titleTextView.setTextAlignment(TEXT_ALIGNMENT_VIEW_START);
            }
        }catch (Exception e){
        ...
        if (lang.equals("fa")) {
            titleTextView.setTypeface(AndroidUtilities.getOriginalTypeface("fonts/IRANSansMedium.ttf"));
        }
        addView(titleTextView, 1, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.LEFT | Gravity.TOP));
    }
    ...
    public void setTitle(CharSequence value) {
        ...
        titleTextView.setPadding(0,15,0,0);
        if (titleTextView != null) {
            lastTitle = value;
    ...
    public ActionBarMenu createMenu() {
        ...
        addView(menu, 1, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.MATCH_PARENT, Gravity.RIGHT));
        return menu;
    }
        ...
        if (titleTextView != null && titleTextView.getVisibility() != GONE || subtitleTextView != null && subtitleTextView.getVisibility() != GONE) {
            ...
            if (titleTextView != null && titleTextView.getVisibility() != GONE) {
                if(Setting.getCurrentFont().contains("IRAN")) {
                    titleTextView.setTextSize(!AndroidUtilities.isTablet() && getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE ? 15 : 17);
                } else {
