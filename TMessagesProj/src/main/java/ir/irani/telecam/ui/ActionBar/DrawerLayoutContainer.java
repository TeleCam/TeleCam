...
public class DrawerLayoutContainer extends FrameLayout {
    ...
    public static boolean isEnabletoTouch=false;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
       //if(!isEnabletoTouch) return false;
//        if (ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE)&&!drawerOpened&&!startedTracking){
//            int max=AndroidUtilities.getRealScreenSize().x;
//            if(ev.getX()>60)return false;
//        }
        if (!parentActionBarLayout.checkTransitionAnimation()) {
            if (drawerOpened && ev != null && ev.getX() > drawerPosition && !startedTracking) {
                ...
                return true;
            }else if (Setting.getProTelegram() && ev != null && ev.getX() > 150.0f && drawerPosition == 0.0f) {
                return false;
            }
            if (allowOpenDrawer && parentActionBarLayout.fragmentsStack.size() == 1) {
                if (ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) && !startedTracking && !maybeStartTracking) {
                    ...
                    startedTrackingX = (int) ev.getX();
                    //Log.e("StartX:","X=" +startedTrackingX);
