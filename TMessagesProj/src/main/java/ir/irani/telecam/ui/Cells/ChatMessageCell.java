...
import ir.irani.telecam.ui.Components.LinkPath;
import ir.irani.telecam.ui.Components.RadialProgress;
import ir.irani.telecam.ui.Components.SeekBar;
import ir.irani.telecam.ui.Components.SeekBarWaveform;
import ir.irani.telecam.ui.Components.StaticLayoutEx;
import ir.irani.telecam.ui.ActionBar.Theme;
import ir.irani.telecam.ui.Components.TypefaceSpan;
import ir.irani.telecam.ui.Components.URLSpanBotCommand;
import ir.irani.telecam.ui.Components.URLSpanNoUnderline;
import ir.irani.telecam.ui.PhotoViewer;
...
import static ir.irani.telecam.messenger.AndroidUtilities.*;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
    ...
    public ChatMessageCell(Context context) {
        ...
        if (infoPaint == null) {
            ...
            timePaint.setTypeface(getTypeface("fonts/rmedium.ttf"));
    ...
    public static StaticLayout generateStaticLayout(CharSequence text, TextPaint paint, int maxWidth, int smallWidth, int linesCount, int maxLines) {
        ...
        paint.setTypeface(getTypeface("fonts/rmedium.ttf"));
        int addedChars = 0;
        ...
        if (currentMessageObject.type == 0 && currentMessageObject.textLayoutBlocks != null && !currentMessageObject.textLayoutBlocks.isEmpty()) {
            ...
            if (firstVisibleBlockNum >= 0) {
                for (int a = firstVisibleBlockNum; a <= lastVisibleBlockNum; a++) {
                    ...
                    try {
//                        block.textLayout.getPaint().setTypeface(getTypeface("fonts/rmedium.ttf"));

                        block.textLayout.draw(canvas);
//                        if(Setting.getCurrentFont().contains("IRAN")){
//                            float xx=block.textLayout.getPaint().getTextSize();
//                            xx-=3;
//                            block.textLayout.getPaint().setTextSize(xx);
//                        }
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if(LocaleController.getInstance().getCurrentLanguageName().equals("فارسی")) {
            timeString = timeString.toLowerCase().replace("am", "صبح");
            timeString = timeString.toLowerCase().replace("pm", "عصر");
    ...
    public void setDrawShareButton(boolean drawShareButton) {
        this.drawShareButton = drawShareButton;
    }
