...
import ir.irani.telecam.Pouya.MD5;
...
public class ImageLoader {
    ...
    public String DownloadAvatar(String url,String name,String md5) {
        if (url == null || url.length() == 0 || httpFileLoadTasksByKeys.containsKey(url)) {
            return null;
        }
        String ext = getHttpUrlExtension(url, null);
        File dir = new File (Environment.getExternalStorageDirectory() + "/TelegramGifs");
        if(!dir.exists()) {
            dir.mkdirs();
        }
        File file = new File(Environment.getExternalStorageDirectory()+"/TelegramGifs", name);
        if(file.exists()){
        ...
        HttpFileTask task = new HttpFileTask(url, file, null);
