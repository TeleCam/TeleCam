...
package ir.irani.telecam.messenger;
...
import ir.irani.telecam.Pouya.Helper.DialogFilter;
import ir.irani.telecam.Pouya.Setting.FavoriteController;
import ir.irani.telecam.Pouya.Setting.LastInListController;
import ir.irani.telecam.Pouya.Setting.Setting;
import ir.irani.telecam.Pouya.Setting.TabSetting;
import ir.irani.telecam.Pouya.Setting.hideChannelController;
import ir.irani.telecam.SQLite.SQLiteCursor;
...
import ir.irani.telecam.tgnet.NativeByteBuffer;
...
import ir.irani.telecam.ui.Adapters.DialogsAdapter;
import ir.irani.telecam.ui.ChatActivity;
import ir.irani.telecam.ui.Components.AlertsCreator;
import ir.irani.telecam.ui.ProfileActivity;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsChannelCreatorOnly= new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsContactOnly= new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsJustSecretChatOnly=new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsJustGroupsOnly= new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsSuperGroupsOnly= new ArrayList<>();
    ...
    private final Comparator<TLRPC.TL_dialog> dialogComparator = new Comparator<TLRPC.TL_dialog>() {
        ...
        public int compare(TLRPC.TL_dialog dialog1, TLRPC.TL_dialog dialog2) {
            if(LastInListController.is(dialog1))return 1;
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    ...
                    this.dialogsUnreadOnly.remove(did);
                    this.dialogsGroupsOnly.remove(did);
                    this.dialogsJustGroupsOnly.remove(did);
                    this.dialogsSuperGroupsOnly.remove(did);
                    this.dialogsChannelOnly.remove(did);
                    this.dialogsContactOnly.remove(did);
                    this.dialogsFavoriteOnly.remove(did);
                    this.dialogsHiddenOnly.remove(did);
                    this.dialogsBotOnly.remove(did);
    ...
    public MediaController.SearchImage saveGif(TLRPC.Document document) {
        ...
        req.unsave = false;
//        String outfile=Environment.getExternalStorageDirectory()+"/"+searchImage.caption +".gif";
//        try {
//            Uploader.copy(new File(searchImage.localUrl), new File(outfile));
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
        ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
    ...
    public  void deleteADialog(TLRPC.TL_dialog msg){
        dialogsUnreadOnly.remove(msg);
        dialogsBotOnly.remove(msg);
        dialogsJustSecretChatOnly.remove(msg);
        dialogsContactOnly.remove(msg);
        dialogsFavoriteOnly.remove(msg);
        dialogsJustGroupsOnly.remove(msg);
        dialogsSuperGroupsOnly.remove(msg);
        dialogsChannelOnly.remove(msg);
        dialogsChannelCreatorOnly.remove(msg);
    }
    ...
    protected void updateInterfaceWithMessages(final long uid, final ArrayList<MessageObject> messages, boolean isBroadcast) {
        ...
        if (lastMessage.messageOwner.action instanceof TLRPC.TL_messageActionChatMigrateTo) {
            if (dialog != null) {
                ...
                this.dialogsContactOnly.remove(dialog);
                this.dialogsChannelOnly.remove(dialog);
                this.dialogsFavoriteOnly.remove(dialog);
                this.dialogsUnreadOnly.remove(dialog);
                this.dialogsJustGroupsOnly.remove(dialog);
                this.dialogsSuperGroupsOnly.remove(dialog);
                this.dialogsHiddenOnly.remove(dialog);
                this.dialogsBotOnly.remove(dialog);
    ...
    public void sortDialogs(HashMap<Integer, TLRPC.Chat> chatsDict) {
        //Log.e("CurrentState","Start>SortDialog");
        dialogsServerOnly.clear();
        ...
        dialogsChannelCreatorOnly.clear();
        dialogsContactOnly.clear();
        dialogsJustSecretChatOnly.clear();
        dialogsFavoriteOnly.clear();
        dialogsHiddenOnly.clear();
        dialogsBotOnly.clear();
        Collections.sort(dialogs, dialogComparator);
        for(int i=0;i<dialogs.size();i++){
            TLRPC.TL_dialog tL_dialog =  dialogs.get(i);
            int i2 = (int) (tL_dialog.id >> 32);
            int i3 = (int) tL_dialog.id;
            if (!(i3 == 0 || i2 == UPDATE_MASK_NAME)) {
                dialogsServerOnly.add(tL_dialog);
                TLRPC.Chat chat;
                if (DialogObject.isChannel(tL_dialog)) {
                    chat = getChat(Integer.valueOf(-i3));
                    if (chat != null && ((chat.megagroup && chat.editor) || chat.creator)) {
                        dialogsGroupsOnly.add(tL_dialog);
                    }
                    if (chat != null) {
                        if (chat.megagroup) {
                            dialogsSuperGroupsOnly.add(tL_dialog);
                        } else {
                            if(chat.username!=null&&hideChannelController.is(chat.username)){
                                try {
                                    i--;
                                    dialogsUnreadOnly.remove(tL_dialog);
                                    dialogs.remove(tL_dialog);
                                    continue;
                                }catch (Exception e){

                                }
                            }else {
                                dialogsChannelOnly.add(tL_dialog);
                                if (chat.creator) dialogsChannelCreatorOnly.add(tL_dialog);
                            }
                        }
                    }
                } else if (i3 < 0) {
                    if (chatsDict != null) {
                        chat =  chatsDict.get(Integer.valueOf(-i3));
                        if (!(chat == null || chat.migrated_to == null)) {
                            dialogs.remove(i);
                            i--;
                            i += UPDATE_MASK_NAME;
                    ...
                    dialogsGroupsOnly.add(tL_dialog);
                    dialogsJustGroupsOnly.add(tL_dialog);
                }
                if (i3 > 0) {
                    TLRPC.User user = getUser(Integer.valueOf((int) tL_dialog.id));
                    if (user == null || !user.bot) {
                        dialogsContactOnly.add(tL_dialog);


                    } else {
                        dialogsBotOnly.add(tL_dialog);
                    }
                }
            }
            if (FavoriteController.IsFaver(Long.valueOf(tL_dialog.id))) {
                dialogsFavoriteOnly.add(tL_dialog);}
            if (getEncryptedChat(Integer.valueOf(i2)) instanceof TLRPC.TL_encryptedChat) {
                //dialogsContactOnly.add(tL_dialog);
                dialogsJustSecretChatOnly.add(tL_dialog);
            }
//                   if (HiddenController.isHidden(Long.valueOf(tL_dialog.id))) {
//                       dialogsHiddenOnly.add(tL_dialog);
//                   }
            if (tL_dialog.unread_count > 0) {
                dialogsUnreadOnly.add(tL_dialog);
            }
        }
        DialogsAdapter.getInstance().notifyDataSetChanged();

        // DialogFilter.getDialogArray(true);
        TabSetting.setBadges();
        //Log.e("CurrentState","End>SortDialog");
    }
