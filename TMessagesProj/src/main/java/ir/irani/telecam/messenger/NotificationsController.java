...
import ir.irani.telecam.ui.PopupNotificationActivity;
...
public class NotificationsController {
    ...
    private String getStringForMessage(MessageObject messageObject, boolean shortMessage) {
        ...
        String msg = null;
        if ((int) dialog_id == 0 || AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter||HiddenController.isHidden(dialog_id)) {
    ...
    private void showExtraNotifications(NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast) {
        ...
        for (int b = 0; b < sortedDialogs.size(); b++) {
            ...
            TLRPC.FileLocation photoPath = null;
            if (AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter||HiddenController.IsHidden(dialog_id)) {
                name = LocaleController.getString("AppName", R.string.AppName);
            } else {
                if (chat != null) {
                    name = chat.title;
            ...
            if (!ChatObject.isChannel(chat) && !AndroidUtilities.needShowPasscode(false) && !UserConfig.isWaitingForPasscodeEnter&&!HiddenController.IsHidden(dialog_id)) {
