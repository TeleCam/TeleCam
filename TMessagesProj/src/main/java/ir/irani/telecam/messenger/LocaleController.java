...
import ir.irani.telecam.messenger.time.FastDateFormat;
        ...
        try {
            ...
            if (year != dateYear) {
                ...
                if(dayDiff == 0 || dayDiff == -1 && (int)(System.currentTimeMillis() / 1000) - date < 60 * 60 * 8) {
                    String t= getInstance().formatterDay.format(new Date(date * 1000));
                    if(LocaleController.getInstance().getCurrentLanguageName().equals("فارسی")) {
                        t=t.toLowerCase().replace("am","صبح");
                        t=t.toLowerCase().replace("pm","عصر");
                    }
                    return t;
